...
**Maps to JD:** Windows 11 SOE; security baselines; device hardening.
...
## Known lab limitation (to document honestly)
**Credential Guard** requires nested virtualisation exposed to the guest (Proxmox CPU type `host`). If activation fails on the VM, it is documented as a known lab constraint — configured in policy, activation pending hardware support.

## Skills
Security baselines · BitLocker + Entra key escrow · Microsoft Defender · device hardening · SOE documentation

**Résumé line:** *Built a hardened Windows 11 SOE with Microsoft security baselines, BitLocker key escrow to Entra, and a documented, versioned build standard.*
