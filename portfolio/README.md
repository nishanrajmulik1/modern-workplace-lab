...
```
Proxmox host (Intel Core Ultra 9 285HX · 192 GB RAM)
  ├─ pfSense ........ firewall / router / DHCP — VLAN10 LAN, VLAN20 MGMT, VLAN30 DMZ
  ├─ DC01 ........... Windows Server 2022 — Active Directory + DNS (corp.mylogigroup.online)
  ├─ SRV01 .......... Windows Server 2022 — Entra Connect Sync + app packaging
  └─ CLIENT01/04 .... Windows 11 Enterprise — Autopilot/Intune managed endpoints
                              │
                              ▼
...
|---|---------|--------------------|--------|
| 0 | [Foundation — Tenant, Licensing & Groups](./00-foundation) | M365 E3, Entra ID P1, custom domain, dynamic device groups | ✅ Complete |
| 1 | [Hybrid Identity — Entra Connect](./01-hybrid-identity) | Entra Connect Sync, password hash sync, Hybrid Entra Join | ✅ Complete |
| 2 | [Conditional Access Baseline](./02-conditional-access) | Zero Trust, MFA, legacy-auth block, break-glass, device compliance | ✅ Complete |
| 3 | [Intune Enrollment + Autopilot](./03-intune-autopilot) | Windows Autopilot (user-driven), Intune MDM, ESP, compliance | ✅ Complete |
| 4 | [Windows 11 SOE + Security Baselines](./04-windows11-soe) | Security baselines, BitLocker + Entra key escrow, Defender, hardening | 🔜 In progress |
...
**Infrastructure:** Proxmox VE, pfSense, VLAN segmentation, Windows Server 2022, Active Directory, DNS
...
This lab wasn't frictionless — and that's the point. Documented, solved issues include:

- **VLAN trunk misconfiguration** — pfSense LAN NIC was tagged as an access port instead of a trunk; corrected with `trunks=10;20;30` on the Proxmox VM so VLANs 10/20/30 reach pfSense tagged.
- **Kerberos failure from VM clock skew** — Entra Connect LDAP bind failed with `0x31 Invalid Credentials` because SRV01's clock had drifted ~1 day from DC01; fixed time hierarchy and anchored the PDC to external NTP.
...
*Lab owner: Nishan Rajmulik · Sydney, NSW · [LinkedIn](https://linkedin.com/in/nrajmulik) · [GitHub](https://github.com/nishanrajmulik1)*
