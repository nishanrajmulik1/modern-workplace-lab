...
**Maps to JD:** Application Packaging & Deployment (Win32, MSI, EXE, PowerShell).
...
- Three install methods covered across the apps:
...
> **Note on PSADT in Intune:** every PSADT package shows `Name: Deploy-Application.exe` in Intune regardless of payload — that's the PSADT *launcher*, not the installer. The actual install logic lives inside `Deploy-Application.ps1`.
...
**Résumé line:** *Package Win32 apps with PSADT — building detection rules, requirement rules, and staged ring deployments across MSI, EXE, and PowerShell install methods.*
