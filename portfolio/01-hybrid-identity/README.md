...
Cloud Sync is Microsoft's lightweight provisioning agent, but it **cannot configure Hybrid Entra Join**. Because this project requires hybrid join, the full **Entra Connect Sync** was used. Being able to explain *why* one was chosen over the other is a practical, field-relevant distinction.
...
- **Kerberos `0x31 Invalid Credentials`** during the Entra Connect LDAP bind — root cause was ~24h **VM clock skew** between SRV01 and DC01 (Kerberos tolerates ≤5 min). Fixed the domain time hierarchy and anchored the PDC emulator to external NTP.
...
**Résumé line:** *Implemented hybrid identity with Entra Connect Sync — password hash sync and Hybrid Entra Join validated via `dsregcmd` — against an on-prem AD forest.*
